...
//	}
	...
	public static void printTransaction(PosTransaction transaction) {
		try {
			...
			TicketPrintProperties printProperties = new TicketPrintProperties("*** ВРЕМЯ ПЛАТИТЬ:) ***", true, true, true);
			printProperties.setPrintCookingInstructions(false);
	...
	public static JasperPrint createKitchenPrint(KitchenTicket ticket) throws Exception {
		...
		if(StringUtils.isNotEmpty(ticket.getCustomerName())) {
			map.put("customer", "Customer: " + ticket.getCustomerName());
		}
	...
	private static void printQuitely(JasperPrint jasperPrint) throws JRException {
		try {
			...
			JRTextExporter txt_exporter = new JRTextExporter();

			txt_exporter.setParameter(JRTextExporterParameter.PAGE_WIDTH, 80);
			txt_exporter.setParameter(JRTextExporterParameter.PAGE_HEIGHT, 1400);
			txt_exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
			String txt_file = "/tmp/" + java.util.UUID.randomUUID().toString();
			txt_exporter.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, txt_file);
			txt_exporter.exportReport();

    		String printer_name = jasperPrint.getProperty("printerName");

			String command_line = "cat " + txt_file + " | txt2escpos.py | lp -d" + printer_name + " -o raw ";
			Process p = Runtime.getRuntime().exec(new String[]{"bash", "-c", command_line});
            p.waitFor();
		} catch (Exception x) {
			x.printStackTrace();
		}
